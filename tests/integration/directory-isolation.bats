...
@test "spec skill includes directory management step" {
    ...
    grep -q "manage-spec-directory" "$PROJECT_ROOT/framework/skills/csf/spec/SKILL.md"

    # Check it explains directory management workflow
    grep -q "Directory Management" "$PROJECT_ROOT/framework/skills/csf/spec/SKILL.md"
    grep -q "Update (u) or Create new (n)" "$PROJECT_ROOT/framework/skills/csf/spec/SKILL.md"
}
...
@test "directory isolation maintains backward compatibility" {
    ...
    grep -q "CSF_DIR/spec\.md.*direct file or symlink" "$PROJECT_ROOT/framework/skills/csf/spec/SKILL.md"
}
