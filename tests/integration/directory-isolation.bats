...
@test "spec skill includes directory management step" {
    ...
    grep -q "manage-spec-directory" "$PROJECT_ROOT/skills/csf/spec/SKILL.md"

    # Check it explains directory management workflow
    grep -q "Directory Management" "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
    grep -q "Update (u) or Create new (n)" "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
}
...
@test "directory isolation maintains backward compatibility" {
    ...
    grep -q "CSF_DIR/spec\.md.*direct file or symlink" "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
}
...
@test "framework validation recognizes manage-spec-directory agent" {
    ...
    cd "$PROJECT_ROOT"
    ./scripts/validate-framework.sh 2>&1 | grep -q "Found 12 agent files"
