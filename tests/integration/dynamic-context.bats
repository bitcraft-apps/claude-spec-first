...
@test "spec exists check returns yes when file present" {
    mkdir -p "$TEST_DIR/.claude/.csf"
    touch "$TEST_DIR/.claude/.csf/spec.md"
    cd "$TEST_DIR"
    run bash -c 'test -f .claude/.csf/spec.md && echo "yes" || echo "no"'
    [ "$output" = "yes" ]
}
...
@test "implementation summary shows content when file exists" {
    mkdir -p "$TEST_DIR/.claude/.csf"
    echo "summary line 1" > "$TEST_DIR/.claude/.csf/implementation-summary.md"
    cd "$TEST_DIR"
    run bash -c 'test -f .claude/.csf/implementation-summary.md && head -20 .claude/.csf/implementation-summary.md || echo "none"'
...
@test "implementation summary caps at 20 lines" {
    mkdir -p "$TEST_DIR/.claude/.csf"
    for i in $(seq 1 30); do echo "line $i"; done > "$TEST_DIR/.claude/.csf/implementation-summary.md"
    cd "$TEST_DIR"
    run bash -c 'test -f .claude/.csf/implementation-summary.md && head -20 .claude/.csf/implementation-summary.md || echo "none"'
    [ "${#lines[@]}" -eq 20 ]
}
...
@test "spec skill has Project Context with dynamic injection" {
    grep -q '## Project Context' "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
    grep -q '!`git branch --show-current 2>/dev/null`' "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
    grep -q '!`git log --oneline -5 2>/dev/null`' "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
    grep -q '!`git status --short 2>/dev/null | head -20`' "$PROJECT_ROOT/skills/csf/spec/SKILL.md"
}

@test "implement skill has Project Context with dynamic injection" {
    grep -q '## Project Context' "$PROJECT_ROOT/skills/csf/implement/SKILL.md"
    grep -q '!`git branch --show-current 2>/dev/null`' "$PROJECT_ROOT/skills/csf/implement/SKILL.md"
    grep -q '!`test -f .claude/.csf/spec.md && echo "yes" || echo "no"`' "$PROJECT_ROOT/skills/csf/implement/SKILL.md"
}

@test "document skill has Project Context with dynamic injection" {
    grep -q '## Project Context' "$PROJECT_ROOT/skills/csf/document/SKILL.md"
    grep -q '!`test -f .claude/.csf/implementation-summary.md' "$PROJECT_ROOT/skills/csf/document/SKILL.md"
}
