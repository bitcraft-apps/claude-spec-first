...
### Changed
- Migrate commands to skills format — `framework/commands/*.md` → `skills/*/SKILL.md` with YAML frontmatter (#95)
- Update `validate-framework.sh` to validate skills in repo mode, commands in installed mode
- Update `install.sh` to install from `skills/*/SKILL.md` instead of `framework/commands/`
